...
from Source.Parameters.parameters import Parameters
from superjeu import GUIflag
...
class MarbleFrame(wx.Frame, MarbleDisplayMixin, MarbleInputMixin, MarbleMenuBarMixin):
    ...
    def __init__(self):
        ...
        self._api = Parameters(notify_gui=self.callToObserver)
        ...
        # Creating a slider and a display zone.
        parameter_text = self._setupParameterTextMixin(main_panel, self._createAcceleratorTable())

        parameter_slider = self._setupSliderMixin(main_panel, "Changer la taille du texte")

        # Setting the layout of the window.
        parameter_sizer = wx.BoxSizer(wx.HORIZONTAL)
        parameter_sizer.AddStretchSpacer(prop=15)
        parameter_sizer.Add(window=parameter_text, proportion=169)
        parameter_sizer.AddStretchSpacer(prop=39)
        parameter_sizer.Add(window=parameter_slider, proportion=169)
        parameter_sizer.AddStretchSpacer(prop=15)

        # TODO replace all dat
        left_sizer = wx.BoxSizer(wx.VERTICAL)
        left_sizer.AddStretchSpacer(prop=10)
        left_sizer.Add(sizer=parameter_sizer, proportion=0, flag=wx.EXPAND)
        left_sizer.AddStretchSpacer(prop=20)
        left_sizer.Add(window=display_text, proportion=0, flag=wx.EXPAND)
        left_sizer.AddStretchSpacer(prop=30)
